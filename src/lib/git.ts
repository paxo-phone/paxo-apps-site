...
import { escapeRegex, readFromStream } from './index'
import path from 'path'
import axios from 'axios'

export default class Git {
    public static async getRefHeadCommit(remoteurl: string, ref: string): Promise<string> {
        ...
        if (lines.length < 2 || !lines[1]) {
            throw new Error("Cannot find this branch")
        }

        return lines[1]
    }

    public static async listBranches(remoteurl: string): Promise<string[]> {
        const remote = new URL(remoteurl)
        remote.pathname = path.join(remote.pathname, '/info/refs')
        remote.searchParams.set('service', 'git-upload-pack')
